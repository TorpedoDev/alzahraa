...
@section('content')
    ...
    <div class="row row-sm">
        <div class="col-xl-12">
            <div class="card">
                <div id="nav" class="card-header pb-0">
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        @endif
                        <h4 class="card-title mg-b-0"> عرض مدة اللبن المستلم</h4>
                    </div>
                    <p class="tx-12 tx-gray-500 mb-2"></p>
                    <br>
                    <div id="nav" style="float: left; width:100%;" class="button-group">
                        <a style=" margin-left: 20px" class="btn btn-warning" href="{{ url()->previous() }}">
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
                        ...
                        <button onclick="window.print(); return false;" class="btn btn-secondary">
                            ...
                            <a class="btn btn-primary"  href="" onclick="event.preventDefault(); document.getElementById('salpay').submit(); " style="margin-right: 20px;">
                                سداد  
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-check-lg" viewBox="0 0 16 16">
                                    <path d="M12.736 3.97a.733.733 0 0 1 1.047 0c.286.289.29.756.01 1.05L7.88 12.01a.733.733 0 0 1-1.065.02L3.217 8.384a.757.757 0 0 1 0-1.06.733.733 0 0 1 1.047 0l3.052 3.093 5.4-6.425z"/>
                                  </svg>
                            </a>
                            <form style="display: none;" id="salpay" action="{{route('cus.paysal' , ['id' => $customer->id , 'from' => $from , 'to' => $to])}}" method="post">
                                @csrf
                            </form>
                            @else
                            <a class="btn btn-primary"  href="" onclick="event.preventDefault(); document.getElementById('salpay').submit(); "  style="margin-right: 20px;">
                            ...
                            <form style="display: none;" id="salpay" action="{{route('cus.paysal' , $customer->id)}}" method="post">
                                @csrf
                            </form>
                            @endif 
                ...
                @if (isset($all) && count($all) > 0)
                            ...
                            <table id="print" class="table table-striped mg-b-0 text-md-nowrap">
                                <thead>
                                    ...
                                    <tr>
                                        ...
                                        <th style="font-weight: bold; font-size: 19px;">الارضيات</th>
                                    ...
                                    <tr>
                                      ...
                                      <h5>   تم خصم : {{$customer->debts->where('is_paid' , 0)->sum('paid')}}  </h5>
                                       ...
                                       <h5>    باقي : {{$customer->debts->where('is_paid' , 0)->sum('rest') }}  </h5>
                                        ...
                                        @if  ($customer->deposit->count() > 0 )
                                            
                                        <td>
                                           <h5> اجمالي الأرضيات : {{ $customer->deposit->sum('value')}} </h5>
                                            
                                      <h5>   تم خصم : {{$customer->deposit->sum('paid')}}  </h5>
                                         
                                       <h5>    باقي : {{$customer->deposit->sum('rest') }}  </h5>
                                        </td>
                                        @else
                                        <td></td>
                                        @endif
                   ...
                   <div class="card-body">
                    <div class="alert alert-info" style="width: 100%">
                        هذا العميل ليس له أي مدة لم يتم سدادها
                    </div>
                   </div>
                @endif
            </div>
