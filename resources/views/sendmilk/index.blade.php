...
<div class="row row-sm">
  ...
  <path d="M12.736 3.97a.733.733 0 0 1 1.047 0c.286.289.29.756.01 1.05L7.88 12.01a.733.733 0 0 1-1.065.02L3.217 8.384a.757.757 0 0 1 0-1.06.733.733 0 0 1 1.047 0l3.052 3.093 5.4-6.425z"/>
                    ...
                    @endif
                    <h4 class="card-title mg-b-0">  طلبيات اللبن</h4>
                </div>
                ...
                <div id="nav" style="float: left;">
                    ...
                    <a class="btn btn-primary" href="{{route('sendmilk.create')}}" style="margin-right: 20px;">
                        تسليم لبن
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="print" class="table table-striped mg-b-0 text-md-nowrap">
                        <thead>
                            <tr>
                                ...
                                <th>ملاحظات التسليم</th>
                                <th>المسئول عن التسليم</th>
                                <th>العمليات</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($sendMilk as $index => $milk)
                            <tr>
                                <th scope="row">{{$sendMilk->firstItem() + $index}}</th>
                                <td>{{$milk->trader->name}}</td>
                                <td>
                   ...
                   <td>{{$milk->user->name}}</td>
                                ...
                                <td>
                                    <div class="row">
                                        <a style="margin-left: 32px;" href="{{ route('sendmilk.edit', $milk->id) }}">
                                            <i class="fa fa-edit text-info" title="تعديل"></i>
                                        </a>

                                        <form action="{{ route('sendmilk.destroy' , $milk->id) }}" method="post">
                                            @method('delete')
                                            @csrf
                                            <button style="border:none; background:none;" type="submit" onclick="return confirm('هل تريد الحذف بالفعل ؟ إذا كنت تريد الحذف فسيتم خصم سعر الطلبية من ديون التاجر')">
